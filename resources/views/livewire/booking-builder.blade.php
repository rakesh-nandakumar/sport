<div class="grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
                    ...
                    <div>
                        ...
                        <div class="mt-3 space-y-2.5">
                            @foreach($methods as $m)
                                @php($on = $this->canUsePaymentMethod($m))
                                @php($restricted = $m === \App\Enums\PaymentMethod::PayAtVenue && auth()->user()?->isPayAtVenueBanned())
    ...
    @if($showPayAtVenueWarning && $quote)
        <div class="fixed inset-0 z-[100001] flex items-end justify-center bg-black/70 p-0 sm:items-center sm:p-4"
             ...
             x-data="{ init() { document.body.classList.add('overflow-hidden') }, destroy() { document.body.classList.remove('overflow-hidden') } }"
             @keydown.escape.window="$wire.closePayAtVenueWarning()">
            <div class="max-h-[94vh] w-full overflow-y-auto rounded-t-3xl bg-white shadow-2xl sm:max-w-2xl sm:rounded-3xl" role="dialog" aria-modal="true" aria-labelledby="pay-at-venue-warning-title">
                <div class="sticky top-0 z-10 flex items-start justify-between border-b border-gray-100 bg-white/95 px-6 py-4 backdrop-blur">
                    <div>
                        <p class="text-xs font-bold uppercase tracking-[.18em] text-amber-700">Final confirmation</p>
                        <h2 id="pay-at-venue-warning-title" class="display mt-1 text-3xl text-gray-900">Pay at Venue</h2>
                    </div>
                    <button type="button" wire:click="closePayAtVenueWarning" class="text-gray-400 hover:text-gray-700" aria-label="Close"><i class="fa-solid fa-xmark text-xl"></i></button>
                </div>
