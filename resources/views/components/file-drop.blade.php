...
<div
    x-data="{
        ...
        takePhoto() { this.$refs.camera?.click() },
        matches(file) {
...
>
    ...
    @if($camera)
        <input
            ...
            capture="environment"
            class="hidden"
            x-on:change="handle($event.target.files)"
        >
    @endif
    ...
    <span class="ep-drop__text">
        ...
        @if($camera)
            <button type="button" x-on:click.stop="takePhoto()" class="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-brand hover:underline">
