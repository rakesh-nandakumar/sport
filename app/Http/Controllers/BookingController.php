...
class BookingController extends Controller
{
    ...
    /** NIC photos are private identity records, available only to the booking customer, venue, and admins. */
    public function identityDocument(Request $request, Booking $booking, string $side): StreamedResponse
    {
        $this->authorizeView($request, $booking);
        abort_unless(in_array($side, ['front', 'back'], true), 404);

        $path = $side === 'front' ? $booking->nic_front_path : $booking->nic_back_path;
